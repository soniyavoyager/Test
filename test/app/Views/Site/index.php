...
<div class="table-responsive">
    ...
    <tbody>
      ...
      foreach ($Site as $material):?>
		...
		$PlotModel->where('id',$material['plot']);
		$Plotnw = $PlotModel->findAll();
$Plotnwname = $Plotnw[0]['name'];
	;
		...
		$cmpnyModel->where('id',$material['compny']);
		$compny = $cmpnyModel->findAll();
		
		?>
        
		  <td><?=$compny[0]['cmpny_name']?></td>
          <td>
   ...
   if($unit1[0]['Site_m_edit']=='1') 
   { ?> 
          
              
            <a class="btn btn-sm text-primary" title="Edit" data-toggle="modal" data-target="#material-edit-Modal" data-name="<?=$material['name']?>" data-href="<?=base_url('/Site/edit/'.$material['id'])?>">
              <i class="far fa-edit"></i>
            </a>
             <?php } if($unit1[0]['Site_m_delete']=='1') { ?>
            
            <a class="btn btn-sm text-danger" title="Delete" data-toggle="modal" data-target="#material-delete-Modal" data-href="<?=base_url('/Site/delete/'.$material['id'])?>" data-name="<?=$material['name']?>">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Site Master</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <?php echo form_open(base_url('/Site/add'), [
          'method' => 'post',
		  ...
		  <select class="form-control" name="compny" id="exampleInputCategory" style="height: calc(2.25rem + 13px);" required>
		   ...
		   <?php endforeach;?>
		    ...
		    <label>Plot</label>
		     <select class="form-control" name="plot" id="exampleInputCategory" style="height: calc(2.25rem + 13px);" required>
