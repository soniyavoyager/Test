...
class Staff_roles extends BaseController
{
	...
	{
		$staffModel = new Staff_roles_Model();
	   $staffs = $staffModel->findAll();

		return view('staffs_Privileges/index', [
			'page_name' => 'Staff Privileges',
			'page_title' => 'Role',
			'staffs' => $staffs,
		]);
	...
	public function add()
	{
		...
		$position = $this->request->getPost('position');
			 ...
			 } else
			  {
				  $Materials_r_approval='0'; 
			  	 ...
			  	 $work_st_view1 = $this->request->getPost('view11');
				   if($work_st_view1)
			  {
				 $work_st_view=$work_st_view1; 
		...
		$message = 'Privileges  info has been saved.';
		...
		echo json_encode(array(
			'result' => 'success',
			'redirect_url' => base_url('/Staff_roles'),
		));
		exit();
	}
	...
	$db = \Config\Database::connect();
        ...
        $builder =$db->query("SELECT * FROM `staff_roles` WHERE `id`='$id'");
        $staff1 = $builder->getResultArray();
   
 
		return view('staffs_Privileges/edit', 
		[
		'staff1' => $staff1,
		]);
	}
 ...
 $Staff_roles_Model = new Staff_roles_Model();
		$Staff_roles_Model->update($id,$data1);
		   

		

		$message = 'Privileges  info has been updated.';
	...
	public function delete($id)
	{
		...
		return redirect()->to('/Staff_roles');
	}
}
